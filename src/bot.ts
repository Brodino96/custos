...
export class Bot {
  ...
  public async init() {
    ...
    const { data: guild, error: guildError } = await tryCatch(
      this.client.guilds.fetch(this.config.bot.guildId),
    );
    if (guildError) {
      this.logger.error(
        `Failed to fetch the guild with id: ${this.config.bot.guildId}, ${guildError}`,
      );
      return process.exit(0);
    }
  ...
  private async onInteractionCreate(interaction: CommandInteraction) {
    ...
    if (!this.isModerator(source)) {
      ...
      return this.logger.warn(
        `${source.user.username} doesn't have permission to use commands`,
      );
    }
   ...
   */
  private callModule<T extends BotModuleMethod>(
    methodName: T,
    ...args: Parameters<BotModule[T]>
  ): Promise<Awaited<ReturnType<BotModule[T]>>[]> {
    return Promise.all(
      this.modules.map((module) => {
        // This explicit casting helps TypeScript understand how to apply the arguments
        const method = module[methodName] as (
          ...methodArgs: Parameters<BotModule[T]>
        ) => ReturnType<BotModule[T]>;
  ...
  public async addModule(
    module: new (bot: Bot, config: ConfigType) => BotModule,
  ) {
